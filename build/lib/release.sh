...
EOF
        ...
        local -r release_docker_image_tag="${KUBE_DOCKER_REGISTRY-$docker_registry}/${image_name}:${KUBE_DOCKER_IMAGE_TAG-$docker_tag}"
...
function kube::release::package_kube_manifests_tarball() {
  ...
  if [[ -e "${KUBE_ROOT}/cluster/gce/gci/gke-internal-configure.sh" ]]; then
    cp "${KUBE_ROOT}/cluster/gce/gci/gke-internal-configure.sh" "${dst_dir}/"
  fi
  ...
  cp -r "${KUBE_ROOT}/cluster/gce/gci/sysctl/" "${dst_dir}/sysctl/"
  local objects
